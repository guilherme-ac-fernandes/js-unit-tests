...
const createMenu = (object) => {
  let objeto = {
    ...
    pay: () => {
      const arrayProduct = objeto.consumption;
      let sum = 0;
      const { food } = objeto.fetchMenu();
      const { drink } = objeto.fetchMenu();
      const product = { ...food, ...drink };
      for (let w = 0; w < arrayProduct.length; w += 1) {
        if (product[`${arrayProduct[w]}`]) {
          sum += product[`${arrayProduct[w]}`];
        }
      }
      // Fixar número de decimais retornando number (.toFixed retorna string) no link a seguir
      // https://stackoverflow.com/questions/2283566/how-can-i-round-a-number-in-javascript-tofixed-returns-a-string
      return Math.round((sum * 1.1) * 1e2) / 1e2;
    }, 
  };
  return objeto;
}; 
