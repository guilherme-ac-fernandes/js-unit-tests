...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(objetoRetornado.consumption).toMatchObject([]);
    expect(createMenu().consumption).toMatchObject([]);
    ...
    expect(addOrder('coxinha')).toContain('coxinha');
    ...
    const addMultipleOrder = (array) => {
      for (let g = 0; g < array.length; g += 1) {
        objetoRetornado.order(`${array[g]}`);
      }
      return array;
    }
    expect(addMultipleOrder(['coxinha', 'agua', 'sopa', 'sashimi'])).toMatchObject(['coxinha', 'agua', 'sopa', 'sashimi']);
    ...
    expect(addMultipleOrder(['coxinha', 'agua', 'coxinha'])).toMatchObject(['coxinha', 'agua', 'coxinha']);
    ...
    // expect(objetoRetornado.pay()).toBe();
    const pagamento = (arrayProduct) => {
      const objeto = createMenu({ 
        food: { coxinha: 3.9, sopa: 9.9 }, 
        drink: { agua: 3.9, cerveja: 6.9 } 
      });
      for (let r = 0; r < arrayProduct.length; r += 1) {
        objeto.order(`${arrayProduct[r]}`);
      }
      return objeto.pay();
    }
    expect(pagamento(['coxinha', 'agua', 'sopa', 'cerveja'])).toBeCloseTo(27.06);
    expect(pagamento(['sopa', 'cerveja'])).toBeCloseTo(18.48);
    expect(pagamento(['coxinha', 'cerveja'])).toBeCloseTo(11.88);
  });
});
